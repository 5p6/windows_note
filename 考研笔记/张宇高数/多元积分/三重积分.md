...
得到截面面积: $A(z) = \displaystyle \pi f^2(z)$ ,得到它的截面面积为: $A(z) = \displaystyle \pi ab f^2(z)$
...
例如:
1.由 $3x + 4y + 2z = 1$ 和 $x,y,z$ 轴围成的立体 $\Omega : \left\{(x,y)| 3x + 4y + 2z \leq 1 , x\geq 0 , y\geq 0 ,z \geq 0\right\}$,可以写为
...
最后:再次说明一下,先二后一的方法的条件:a.特别简单的图形,每一个截面都是可计算的,b.被积函数简单,通常是最后变量的函数;一般在计算质心的时候很常用;一般都是用先一后二法.
...
利用投影法得到投影 $D_{x,y} = \left\{(x,y)|x^2+(y-1)^2 \leq 1\right\} \rightarrow x^2 + y^2  \leq 2y$,故有:
