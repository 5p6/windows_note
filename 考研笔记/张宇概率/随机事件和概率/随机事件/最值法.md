...
$\left\{\max \left\{X,Y \right\} \leq a \right\} = \left\{X \leq a,Y \leq a \right\}$
最大值小于 $a$ ,说明他们都小于 $a$

##### 2.最大值大于

$\left\{\max \left\{X,Y \right\} > a \right\} = \left\{X > a \cup Y > a \right\}$

最大值大于 $a$ ,说明他们其中有一个大于 $a$.

##### 3.最小值小于

$\left\{\min \left\{X,Y \right\} \leq a \right\} = \left\{X \leq a \cup Y \leq a \right\}$

最小值小于 $a$ ,他们其中一个小于 $a$ 即可.

##### 4.最小值大于
$\left\{\min \left\{X,Y \right\} > a \right\} = \left\{X > a,Y > a \right\}$
最小值大于 $a$ ,说明他们两个都大于 $a$
...
$$\begin{align}
    \left\{\max \left\{X,Y \right\} \leq a \right\} \subseteq \left\{\min \left\{X,Y \right\} \leq a \right\}
\end{align}$$

##### 2.

$$\begin{align}
    \left\{\min \left\{X,Y \right\} > a \right\} \subseteq \left\{\max \left\{X,Y \right\} > a \right\}
\end{align}$$
...
$$\begin{align}
    U = \max \left\{X + Y \right\} =\begin{cases}
        X , X>Y \\ 
        Y, X<Y 
...
$$\begin{align}
    V = \min \left\{X + Y \right\} =\begin{cases}
        Y , X>Y \\ 
        X, X<Y 
    \end{cases} &= \frac{X + Y}{2} - \frac{|X - Y|}{2} 
\end{align}$$
...
#### 四.函数分布
##### 1.乘积
设随机变量 $X,Y$ 相互独立,且他们的期望存在, $U = \max \left\{X,Y\right\} , V = \min\left\{X,Y \right\}$ ,求 $Z = UV$ 的分布函数和期望.
解:
...
##### 2.相加
设随机变量 $X,Y$ 相互独立,且他们的期望存在, $U = \max \left\{X,Y\right\} , V = \min\left\{X,Y \right\}$ ,求 $Z = U + V$ 的分布函数和期望.
...
$$\begin{align}
    Z &= \max \left\{X,Y\right\} + \min\left\{X,Y \right\}\\
...
#### 5.例题
已知 $(X,Y) \thicksim N(0,0;1,1,0)$ ,记 $U = \left\{X,Y\right\} , V = \min\left\{X,Y \right\}, Z = U - V$  ,求
(1) $Z$ 的分布
(2) $(U,V)$ 的分布函数. 
解:
